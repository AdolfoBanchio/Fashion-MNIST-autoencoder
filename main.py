...
for i,(classifier, optimizer) in enumerate(zip(classifiers, optimizers)):
    ...
    with open(f"./results/classifier_{i+1}_results.json", "w") as f:
        ...
        ax[0].plot(trainer.train_loss_incorrect, label='Train Loss Incorrect')
        ...
        ax[1].plot(trainer.train_precision_incorrect, label='Train Precision Incorrect')
        ax[1].plot(trainer.train_precision, label='Train Precision', linestyle='--')
        ax[1].plot(trainer.valid_precision, label='Validation Precision', linestyle='-.')
